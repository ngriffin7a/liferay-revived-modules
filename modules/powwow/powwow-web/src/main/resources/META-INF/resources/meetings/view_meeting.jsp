...
<div class="meeting-container">
	...
	<div class="meeting-url">
		...
		<dd>
			<input id="<portlet:namespace />meetingURL" readonly="readonly" type="text" value="<%= powwowInvitationURLFactory.createInvitationURL(powwowMeetingId, null, (String)request.getAttribute("layoutURL")) %>" />

			<button class="zeroclipboard-button" data-clipboard-text="<%= powwowInvitationURLFactory.createInvitationURL(powwowMeetingId, null, (String)request.getAttribute("layoutURL")) %>" data-copied="<liferay-ui:message key="copied" />" data-hover="<liferay-ui:message key="copy-to-clipboard" />" id="<portlet:namespace />copyButton">
				<i class="icon-copy"></i>
			</button>
		</dd>
	</div>

	<c:if test="<%= powwowServiceProviderRegistry.lookupByType(powwowMeeting.getProviderType()).isSupportsOptionPassword() %>">
		<c:if test="<%= Validator.isNotNull(powwowServiceProviderRegistry.lookupByMeetingId(powwowMeeting.getPowwowMeetingId()).getOptionPassword(powwowMeeting.getPowwowMeetingId())) %>">
			<div class="meeting-password">
				<dt>
					<liferay-ui:message key="meeting-password" />
				</dt>
				<dd>
					<%= powwowServiceProviderRegistry.lookupByMeetingId(powwowMeeting.getPowwowMeetingId()).getOptionPassword(powwowMeeting.getPowwowMeetingId()) %>
				</dd>
			</div>
		</c:if>
	</c:if>

	<c:if test="<%= powwowServiceProviderRegistry.lookupByType(powwowMeeting.getProviderType()).isSupportsJoinByPhone() %>">
		<div class="join-by-phone">
			...
			List<String> joinByPhoneDefaultNumbers = powwowServiceProviderRegistry.lookupByType(powwowMeeting.getProviderType()).getJoinByPhoneDefaultNumbers();
			...
			<dd>
				<div class="join-by-phone-content">
					...
					<div class="access-code">
						<span class="title"><%= LanguageUtil.get(request, powwowServiceProviderRegistry.lookupByType(powwowMeeting.getProviderType()).getJoinByPhoneAccessCodeLabel()) %>:</span> <%= powwowServiceProviderRegistry.lookupByMeetingId(powwowMeetingId).getJoinByPhoneAccessCode(powwowMeetingId) %>
					</div>
	...
	<div class="provider">
		...
		<dd>
			<%= LanguageUtil.get(request, powwowServiceProviderRegistry.lookupByType(powwowMeeting.getProviderType()).getBrandingLabel()) %>
		</dd>
	</div>
...
<aui:script use="aui-base,aui-datatable,aui-toggler,swfdetect">
	...
	<c:if test="<%= powwowServiceProviderRegistry.lookupByType(powwowMeeting.getProviderType()).isSupportsJoinByPhone() %>">
		...
		<%
		Map<String, List<String>> internationalNumbers = powwowServiceProviderRegistry.lookupByType(powwowMeeting.getProviderType()).getJoinByPhoneInternationalNumbers();
