...
<liferay-ui:search-container
	...
	total="<%= tasksEntryLocalService.getTasksEntriesCount(groupId, userId, 0, assigneeUserId, status, assetTagIds, new long[0]) %>"
>
	<liferay-ui:search-container-results
		results="<%= tasksEntryLocalService.getTasksEntries(groupId, userId, 0, assigneeUserId, status, assetTagIds, new long[0], searchContainer.getStart(), searchContainer.getEnd()) %>"
	/>
		...
		>
			...
			<div class="result-data">
					...
					<span><liferay-ui:message key="assignee" />:
						<c:choose>
							<c:when test="<%= tasksEntry.getAssigneeUserId() > 0 %>">
								<%= PortalUtil.getUserName(tasksEntry.getAssigneeUserId(), "N/A") %>
							</c:when>
							<c:otherwise>
				...
				<c:if test='<%= !tabs1.equals("i-have-created") %>'>
					<span><liferay-ui:message key="reporter" />: <%= PortalUtil.getUserName(tasksEntry.getUserId(), "N/A") %></span>
				</c:if>
			</div>
		</liferay-ui:search-container-column-text>
